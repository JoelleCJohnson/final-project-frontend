...
export default function DisplayWishlist() {
    ...
    return (
        <section className="max-w-sm mx-auto flex flex-col items-center justify-center border bg-red-600 border-2 rounded-lg m-4 p-2 col-start-1 ">
            ...
            {/* <ItemCards wishlist={wishlist} /> */}

                {!wishlist ?
                    <h2>Loading...</h2>
                    :
                    wishlist.map((item) => 
                        <ItemCards key={item.listid} itemname={item.itemname} itemlink={item.itemlink} itemprice={item.itemprice} listid={item.listid} ispurchased={item.ispurchased}
                                    />
                        // const thisItem = item
